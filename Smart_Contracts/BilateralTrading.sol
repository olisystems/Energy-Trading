...
contract BilateralTrading is mortal{
    ...
    event NewStock(address saccount, uint64 samount, uint8 smrate, uint32 speriod, uint32 sbiddingTime);
    ...
    function regStock (uint64 _amount, uint8 _rate, uint32 _period, uint32 _btime) validRequirement(_amount) {
    ...
    function get_stockAmount(address _stock) constant returns (uint64) {
